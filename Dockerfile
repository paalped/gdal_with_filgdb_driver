...
# FILGDB Driver 
# Does not support ENV
RUN apt-get install -y --no-install-recommends \
    make gcc
#RUN wget -c https://github.com/Esri/file-geodatabase-api/blob/master/FileGDB_API_1.5.2/FileGDB_API_RHEL7_64.tar.gz?raw=true &&\
RUN wget -c https://github.com/Esri/file-geodatabase-api/blob/master/FileGDB_API_1.5.2/FileGDB_API-RHEL7-64gcc83.tar.gz?raw=true &&\
    tar -xf FileGDB_API-RHEL7-64gcc83.tar.gz\?raw\=true  &&\ 
    cd FileGDB_API-RHEL7-64gcc83 &&\
